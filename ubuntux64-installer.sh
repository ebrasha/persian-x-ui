...
install_x-ui() {
  ...
  cd /usr/local/
  rm -f x-ui-persian-linux-ubuntu-amd64.zip
  if [[ -e /usr/local/x-ui-persian/ ]]; then
    rm /usr/local/x-ui-persian/ -rf
  fi
  url_zip_file="https://raw.githubusercontent.com/abdal-security-group/persian-x-ui/main/x-ui-persian-linux-ubuntu-amd64.zip"
  wget -N --no-check-certificate -O /usr/local/x-ui-persian-linux-ubuntu-amd64.zip ${url_zip_file}
  unzip x-ui-persian-linux-ubuntu-amd64.zip
  rm -f x-ui-persian-linux-ubuntu-amd64.zip
  cd x-ui-persian
