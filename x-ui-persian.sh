...
# check root
[[ $EUID -ne 0 ]] && LOGE "mistake:  This script must be run with the root user!\n" && exit 1
...
reset_user() {
    confirm "Make sure you want to reset your username and password to ebrasha Is it" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
    ...
    /usr/local/x-ui/x-ui setting -username ebrasha -password ebrasha
    echo -e "Username and password have been reset to ${green}ebrasha${plain}，Please restart the panel now"
    confirm_restart
}
...
reset_config() {
    ...
    echo -e "All panel settings have been reset to default, please restart the panel now and use the default ${green}1366${plain} Panel port "
    confirm_restart
}
...
show_enable_status() {
    check_enabled
    if [[ $? == 0 ]]; then
        echo -e "Whether to start automatically: ${green}是${plain}"
    else
        echo -e "Whether to start automatically: ${red}否${plain}"
    fi
}
...
ssl_cert_issue() {
    echo -E ""
    LOGD "******Instructions for use | Created By Ebrahim Shafiei******"
    LOGI "This script will use the Acme script to apply for a certificate. When using it, you need to ensure:"
    ...
    LOGI "4. The default installation path of the script to apply for a certificate is the /root/cert directory"
